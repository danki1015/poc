...
def calculate_score_crit(datalevel, column_name, numerator_column, denominator_column, weight):
    ...
    datalevel = datalevel.assign(**thresh)

    datalevel[f'{column_name}_score_crit'] = (5 ** (datalevel[numerator_column] / thresh[denominator_column])).apply(lambda x: 1 if x > 5 else (x - 1) / 4)
